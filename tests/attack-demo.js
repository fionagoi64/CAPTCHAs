...
const FRONTEND_WITH_CAPTCHA = "http://localhost:5173/with-captcha";
const NO_CAPTCHA_ENDPOINT = `${BACKEND_BASE}/submit-no-captcha`;
const WITH_CAPTCHA_ENDPOINT = `${BACKEND_BASE}/submit-with-captcha`;
...
async function floodNoCaptcha(total, concurrency) {
  ...
  for (let b = 0; b < batches; b++) {
    ...
    await sleep(50);
    process.stdout.write(`\r[NO-CAPTCHA] progress sent=${sent} ok=${ok} fail=${fail}`);
  }
...
async function attemptWithCaptcha(attempts) {
  ...
  const browser = await puppeteerExtra.launch({ headless: HEADLESS, defaultViewport: null, args: ['--no-sandbox'] });
  const results = [];
  try {
    for (let i = 0; i < attempts; i++) {
      ...
      try {
        await page.goto(FRONTEND_WITH_CAPTCHA, { waitUntil: "networkidle2", timeout: 30000 });
        ...
        try {
          ...
          console.warn("[WITH-CAPTCHA] recaptcha iframe NOT detected (maybe v3/invisible)");
        }

        // try to click checkbox in anchor frame (v2)
        const framesObj = page.frames();
        const anchorFrame = framesObj.find(f => /recaptcha\/api2\/anchor/.test(f.url()));
        if (anchorFrame) {
          ...
          if (checkbox) {
            ...
            console.warn("[WITH-CAPTCHA] anchor frame found but #recaptcha-anchor missing");
          }
        } else {
          console.warn("[WITH-CAPTCHA] anchor frame not found; might be invisible or v3");
        }
        ...
        try {
          await page.waitForFunction(() => {
            try { return window.grecaptcha && typeof window.grecaptcha.getResponse === "function" && window.grecaptcha.getResponse().length > 0; } catch(e) { return false; }
          }, { timeout: 10000 });
        ...
        if (token) {
          ...
          results.push({ attempt: i + 1, tokenObtained: true, submitStatus: resp.status, submitBody: resp.body });
        } else {
          console.log("[WITH-CAPTCHA] no token obtained (likely challenge required)");
          results.push({ attempt: i + 1, tokenObtained: false });
        }

      } catch (err) {
        console.error(`[WITH-CAPTCHA] attempt #${i + 1} error:`, err.message || err);
...
(async () => {
  ...
  const noCaptchaPromise = floodNoCaptcha(NO_CAPTCHA_TOTAL, NO_CAPTCHA_CONCURRENCY);

  // 2) Simultaneously run puppeteer attempts for with-captcha
  const withCaptchaPromise = attemptWithCaptcha(PUPPETEER_ATTEMPTS);

  const [noCaptchaRes, withCaptchaRes] = await Promise.all([noCaptchaPromise, withCaptchaPromise]);

  console.log("\n=== SUMMARY ===");
  console.log("[NO-CAPTCHA] ", noCaptchaRes);
  console.log("[WITH-CAPTCHA] ", { attempts: withCaptchaRes.attempts, successTokens: withCaptchaRes.successTokens, results: withCaptchaRes.results });
