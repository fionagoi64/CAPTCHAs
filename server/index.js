...
const RECAPTCHA_V2_SECRET = process.env.RECAPTCHA_V2_SECRET;
const RECAPTCHA_V3_SECRET = process.env.RECAPTCHA_V3_SECRET;
...
app.post("/submit-with-recaptcha-v2", async (req, res) => {
  ...
  try {
    ...
    if (!v.success)
      return res
        .status(403)
        .json({ ok: false, err: "recaptcha v2 failed", detail: v });
...
app.post("/submit-with-recaptcha-v3", async (req, res) => {
  ...
  try {
    ...
    if (!v.success || (v.score && v.score < 0.5)) {
      ...
      return res
        .status(403)
        .json({ ok: false, err: "recaptcha v3 failed", detail: v });
    }
...
app.post("/submit-with-hcaptcha", async (req, res) => {
  ...
  try {
    ...
    if (!v.success)
      return res
        .status(403)
        .json({ ok: false, err: "hcaptcha failed", detail: v });
